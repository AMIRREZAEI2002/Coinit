...
// Sample data (diverse, mix of internal and external sends)
const rows: TransactionRow[] = [
  {
    id: 1,
    crypto: 'SOL/USDT',
    network: 'Internal',
    time: '2025-07-18 15:20:00',
    status: 'Completed',
    amount: 70,
    txid: 'N/A',
  ...
  },
  {
    ...
    crypto: 'ETH/USDT',
    network: 'Ethereum',
    time: '2025-07-17 07:30:45',
    status: 'Failed',
    amount: 1.8,
    txid: '0xdef34...56f7',
    toAddress: '0x789...4567',
    fee: 0.0025,
    confirmations: '0',
  },
  {
  ...
  },
  {
    ...
    crypto: 'DOT/USDT',
    network: 'Polkadot',
    time: '2025-07-16 14:25:15',
    status: 'Processing',
    amount: 130,
    txid: '0xjkl78...90h9',
    toAddress: '0xdef...2345',
    fee: 0.06,
    confirmations: '1/3',
  },
  {
...
const WalletFundingSectionSend: React.FC = () => {
  ...
  const handleRefresh = () => {
    console.log('Refreshing send data...');
    // Placeholder for API call to refresh data
  };

  const handleExport = () => {
    console.log('Exporting send data...');
    // Placeholder for export functionality (e.g., download CSV)
  };

  const handleCopyTxid = (txid: string) => {
    if (txid === 'N/A') return; // No copying for internal sends
  ...
  const handleLinkTxid = (txid: string) => {
    if (txid === 'N/A') return; // No linking for internal sends
          ...
          <Box sx={{ display: 'flex', alignItems: 'flex-end', gap: 1 }}>
            <Box sx={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <StyledSelect
                ...
                name="crypto-filter"
                aria-label="Filter sends by type"
              >
        ...
        <Box sx={{ maxHeight: 400, overflowY: 'auto' }}>
          {filteredRows.length > 0 ? (
            filteredRows.map((row) => (
              // eslint-disable-next-line @typescript-eslint/ban-ts-comment
              // @ts-ignore
              <TransactionCard key={row.id} variants={cardVariants} initial="hidden" animate="visible" aria-label={`Send ${row.id}`}>
...
export default WalletFundingSectionSend;